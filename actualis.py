...
{log}  {os.environ.get("USER", "root")}:staff  644  5  1024  *  GJ
'''
...
def self_check(c: C, days: int | None = 7) -> int:
    ...
    if not roots:
        print("  No transcript directories on this machine, so there is nothing")
        print("  to read and nothing to verify. Run an agent first.\n")
    ...
    # 2. Sample the transcripts, hash them, scan, hash again.
    sample: list[Path] = []
    for root in roots:
        for f in root.rglob("*.jsonl"):
            sample.append(f)
    ...
    # 3. Nothing appeared, vanished or grew anywhere under the roots.
    ...
    # 4. Where this build is allowed to write, named explicitly.
    ...
    # 5. What the binary itself is, so it can be compared with what was published.
    try:
        me = Path(__file__)
        d = _digest_file(me)
        if d:
    ...
    verdict = (f"{c.ok}All checks passed.{c.off}" if ok
               else f"{c.red}A check failed. Do not trust this build.{c.off}")
    print(f"\n  {verdict}\n")
